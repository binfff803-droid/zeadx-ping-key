...
# CORS là tiện ích phụ (cho phép gọi API từ domain khác) — không phải phần lõi.
# Nếu thư viện flask_cors không cài được trên môi trường deploy vì lý do gì đó,
# app vẫn phải chạy được bình thường thay vì sập toàn bộ, nên bọc trong try/except.
try:
    from flask_cors import CORS
    CORS(app)
except Exception as _cors_err:
    print("CANH BAO: khong the nap flask_cors, bo qua CORS. Loi:", _cors_err)
...
def create_new_key(duration_type, note="", max_devices=1, key_type="premium", lock_ip=False):
    keys = load_keys()
...
def admin_create_key():
    ...
    if duration_type not in ["12h","24h","1d","3d","7d","30d","forever"]:
        duration_type = "12h"
    ...
    for _ in range(quantity):
        key_obj, err = create_new_key(duration_type, note=note, max_devices=max_devices, key_type=key_type, lock_ip=lock_ip)
        if err:
            if created:
...
@app.route("/api/admin/verify-ip/<int:key_id>", methods=["POST"])
@login_required
def admin_verify_ip(key_id):
    """Kiểm tra / gán IP cho một key thật trong keys.json.
    - Nếu key không khoá IP: chấp nhận mọi IP, chỉ đánh dấu đã dùng.
    - Nếu key khoá IP và chưa có IP gán: gán IP hiện tại cho key.
    - Nếu key khoá IP và đã có IP gán: so khớp với IP được gửi lên."""
    data = request.get_json(force=True, silent=True) or {}
    ip = str(data.get("ip", "")).strip()
    if not ip:
        return jsonify({"error": "Thiếu địa chỉ IP"}), 400
    ...
    target.setdefault("lock_ip", False)
    target.setdefault("assigned_ip", None)

    if not target["lock_ip"]:
        target["used"] = True
        save_keys(keys)
        return jsonify({"locked": False, "match": True, "assigned_ip": target["assigned_ip"]})

    if not target["assigned_ip"]:
        target["assigned_ip"] = ip
        target["used"] = True
        save_keys(keys)
        return jsonify({"locked": True, "match": True, "assigned_ip": ip, "just_assigned": True})

    match = (target["assigned_ip"] == ip)
    if match:
        target["used"] = True
        save_keys(keys)
    return jsonify({"locked": True, "match": match, "assigned_ip": target["assigned_ip"]})
    ...
    for item in keys:
        if item.get("code", "").upper() == k:
            ...
            duration_map = {"12h": 12*60*60*1000, "24h": 24*60*60*1000, "1d": 24*60*60*1000,
                             "3d": 3*24*60*60*1000, "7d": 7*24*60*60*1000, "30d": 30*24*60*60*1000,
                             "forever": None}
...
def admin_login():
    ...
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@500;600;700&family=Inter:wght@400;500;600;700&family=Orbitron:wght@700;800;900&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
    <style>
      :root{{
        --bg-0:#020509; --bg-1:#040c18; --navy:#0a1f38;
        --ocean:#0ea5e9; --ocean-2:#38bdf8; --ocean-deep:#0369a1;
        --ice:#e6f6ff; --white:#f7fbff;
        --border: rgba(56,189,248,0.18);
        --surface: rgba(10,25,45,0.55); --surface-2: rgba(4,12,24,0.6);
        --text-dim: rgba(230,246,255,0.55); --text-faint: rgba(230,246,255,0.35);
      ...
      *{{ box-sizing:border-box; margin:0; padding:0; }}
      html,body{{ height:100%; }}
      body{{
        min-height:100vh; font-family:'Inter', sans-serif; color:var(--ice);
        display:flex; align-items:center; justify-content:center; padding:20px;
        background:
          radial-gradient(circle at 18% 15%, rgba(14,165,233,0.22), transparent 45%),
          radial-gradient(circle at 85% 85%, rgba(56,189,248,0.16), transparent 45%),
          radial-gradient(circle at 85% 10%, rgba(52,211,153,0.06), transparent 40%),
          var(--bg-0);
      ...
      body::before{{ width:360px; height:360px; background:rgba(14,165,233,0.28); top:-120px; left:-100px; }}
      body::after{{ width:300px; height:300px; background:rgba(56,189,248,0.2); bottom:-110px; right:-90px; animation-delay:-6s; }}
      @keyframes loginOrb{{ 0%,100%{{ transform:translate(0,0) scale(1);}} 50%{{ transform:translate(24px,-18px) scale(1.08);}} }}

      .login-shell{{ position:relative; width:100%; max-width:396px; animation: loginIn .55s cubic-bezier(.16,1,.3,1); z-index:1;}}
      ...
      .login-card{{
        ...
        padding:40px 34px 32px; box-shadow: var(--shadow-black);
        text-align:center; overflow:hidden;
      }}
      .login-card::before{{
        content:''; position:absolute; inset:0; border-radius:inherit; padding:1px;
        background: linear-gradient(135deg, rgba(14,165,233,0.55), rgba(56,189,248,0.05) 30%, rgba(255,255,255,0.03) 60%, rgba(52,211,153,0.3));
        -webkit-mask: linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0);
        -webkit-mask-composite: xor; mask-composite: exclude; pointer-events:none;
      }}
      .login-badge{{ width:76px; height:76px; margin:0 auto 18px; position:relative; display:flex; align-items:center; justify-content:center; }}
      .login-badge::before{{
        content:''; position:absolute; inset:-10px; border-radius:50%;
        background: conic-gradient(from 0deg, var(--ocean), var(--ocean-2), #34d399, var(--ocean));
        filter:blur(14px); opacity:.5; animation: badgeSpin 6s linear infinite;
      }}
      ...
      .login-badge svg{{ width:34px; height:34px; stroke:var(--ocean-2); }}
      .login-card h1{{ font-family:'Orbitron', sans-serif; font-size:20px; margin:0 0 6px; letter-spacing:.5px; color:var(--white);}}
      .login-card p{{ color:var(--text-dim); font-size:13px; margin:0 0 26px; line-height:1.55; font-family:'Rajdhani',sans-serif; letter-spacing:.3px; }}

      .field-group{{ position:relative; text-align:left; margin-bottom:14px; }}
      .field-group svg{{
        position:absolute; left:14px; top:50%; transform:translateY(-50%);
        width:16px; height:16px; stroke:var(--text-faint); pointer-events:none; transition:stroke .15s ease;
      }}
      .login-card input{{
        width:100%; padding:13px 14px 13px 40px;
        background: var(--surface-2); border:1px solid var(--border); border-radius:11px;
        color:var(--ice); font-size:14px; font-family:'JetBrains Mono', monospace;
        transition: border-color .15s ease, box-shadow .15s ease;
      }}
      .login-card input::placeholder{{ color: var(--text-faint); font-family:'Inter', sans-serif;}}
      .login-card input:focus{{ outline:none; border-color: var(--ocean-2); box-shadow: 0 0 0 3px rgba(56,189,248,0.16); }}
      .field-group input:focus + svg{{ stroke: var(--ocean-2); }}
      .pass-toggle{{
      ...
      .pass-toggle svg{{ width:18px; height:18px; stroke:currentColor; }}
      .pass-toggle:hover{{ color:var(--ice); background: rgba(255,255,255,0.06); }}

      .btn-primary{{
        width:100%; border:none; cursor:pointer; border-radius:11px; margin-top:6px;
        background: linear-gradient(135deg, var(--ocean), var(--ocean-2));
        color:#031018; font-family:'Rajdhani',sans-serif; font-weight:700; font-size:15px;
      ...
      #formMsg{{
        ...
        color:var(--rose); font-size:12.5px; min-height:18px; margin-top:10px;
      }}
      .login-note{{
        display:flex; align-items:flex-start; gap:9px; text-align:left;
        font-size:11.5px; color:var(--text-faint); margin-top:18px; line-height:1.55;
        background: var(--surface-2); border:1px solid var(--border); border-radius:12px; padding:12px 13px;
      }}
      .login-note svg{{ flex-shrink:0; margin-top:1px; width:14px; height:14px; stroke:var(--amber); }}
    ...
    <div class="login-shell" id="loginShell">
      <div class="login-card">
        ...
        <h1>ZEADX PING ADMIN</h1>
        <p>Đăng nhập để quản lý hệ thống key</p>

        <form method="POST" id="loginForm" novalidate>
          <div class="field-group">
            <input id="usernameField" name="username" placeholder="Tài khoản" autocomplete="username">
            <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4.4 3.6-8 8-8s8 3.6 8 8"/></svg>
          </div>
          <div class="field-group">
            <input id="passwordField" name="password" type="password" placeholder="Mật khẩu" autocomplete="current-password" style="padding-right:42px;">
            <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg>
            <button type="button" class="pass-toggle" id="eyeToggle" aria-label="Hiện/ẩn mật khẩu">
        ...
        <div id="formMsg" style="{'display:flex' if error else 'display:none'}">
        ...
        <div class="login-note">
          ...
          <span>Vui lòng không chia sẻ thông tin đăng nhập cho người khác.</span>
        </div>
      </div>
    </div>
    <script>
      ...
      document.getElementById('loginForm').addEventListener('submit', function(e){{
        ...
        if(!u || !p){{
          ...
          msg.querySelector('span').textContent = 'Vui lòng nhập tài khoản và mật khẩu';
      ...
      {'document.getElementById("loginShell").classList.add("shake");' if error else ''}
    </script>
    </body></html>'''
...
def admin_create_key_page():
    ...
    <script>
      function updateExpiryPreview(){{
        ...
        var msMap = {{'12h':12*3600000,'24h':24*3600000,'1d':24*3600000,'3d':3*24*3600000,'7d':7*24*3600000,'30d':30*24*3600000,'forever':null}};
...
<style>
  :root{
    ...
    --shadow-black: 0 24px 48px -12px rgba(0,0,0,0.85), 0 4px 12px rgba(0,0,0,0.6);
    --shadow-black-sm: 0 10px 24px -8px rgba(0,0,0,0.75);
    --radius: 14px;
  }
  *{box-sizing:border-box;}
  ...
  body{
    background:
      radial-gradient(circle at 10% -10%, rgba(129,140,248,0.14), transparent 45%),
      radial-gradient(circle at 90% 0%, rgba(34,211,238,0.10), transparent 40%),
  ...
  .btn-primary{
    ...
    color:#0a0d14; box-shadow: 0 12px 24px -8px rgba(129,140,248,0.55);
  }
  .btn-primary:hover{ filter:brightness(1.08); }
  ...
  .btn-icon{
    ...
    background: var(--surface-2); color:var(--text-dim); border:1px solid var(--border); border-radius:9px;
  }
  ...
  .live-dot{ width:7px; height:7px; border-radius:50%; background:var(--emerald); box-shadow:0 0 0 3px rgba(52,211,153,0.18); animation: blink 2s infinite; }
  ...
  .stat-card{
    ...
    padding:20px; box-shadow: var(--shadow-black); position:relative; overflow:hidden;
  }
  .stat-card::before{ content:''; position:absolute; top:-30%; right:-20%; width:120px; height:120px; border-radius:50%; filter:blur(30px); opacity:.28; }
  .stat-card.total::before{ background: var(--indigo); }
  .stat-card.unused::before{ background: var(--amber); }
  .stat-card.active::before{ background: var(--emerald); }
  .stat-card.used::before{ background: var(--rose); }
  ...
  .stat-card.total .stat-icon{ background:rgba(129,140,248,0.14); color:var(--indigo); }
  .stat-card.unused .stat-icon{ background:rgba(251,191,36,0.14); color:var(--amber); }
  .stat-card.active .stat-icon{ background:rgba(52,211,153,0.14); color:var(--emerald); }
  .stat-card.used .stat-icon{ background:rgba(251,113,133,0.14); color:var(--rose); }
  .stat-trend{ font-size:11px; color:var(--text-faint); }
  .stat-value{ font-size:32px; font-weight:700; font-family:'Space Grotesk', sans-serif; line-height:1; }
  ...
  .tab-btn{
    ...
    font-family:'Inter', sans-serif; font-weight:600; font-size:13.5px;
    padding:12px 6px; display:flex; align-items:center; gap:8px;
  ...
  .panel{
    background: linear-gradient(165deg, var(--surface), var(--surface-2));
    border:1px solid var(--border); border-radius: var(--radius);
    box-shadow: var(--shadow-black); overflow:hidden;
  }
  ...
  .panel-head h2{ font-size:15px; margin:0; font-family:'Space Grotesk', sans-serif; }
  .panel-body{ padding:20px; }

  .field{ margin-bottom:16px; }
  .field label{ display:block; font-size:12px; color:var(--text-dim); margin-bottom:7px; font-weight:600; letter-spacing:.2px;}
  .field input, .field select{
    width:100%; padding:11px 12px; background: var(--surface-2);
  ...
  .slider{ position:absolute; inset:0; background:var(--surface-2); border:1px solid var(--border); border-radius:999px; cursor:pointer; transition:.2s; }
  .slider::before{ content:''; position:absolute; width:16px; height:16px; left:3px; top:2.5px; background:var(--text-faint); border-radius:50%; transition:.2s; }
  .switch input:checked + .slider{ background: rgba(52,211,153,0.25); border-color: var(--emerald); }
  .switch input:checked + .slider::before{ transform: translateX(18px); background: var(--emerald); }
  .full-btn{ width:100%; justify-content:center; margin-top:6px; }

  .table-toolbar{ display:flex; gap:10px; padding:16px 20px; border-bottom:1px solid var(--border); flex-wrap:wrap; }
  .search-box{ flex:1; min-width:180px; position:relative; }
  .search-box input{
    width:100%; padding:10px 12px 10px 34px; background: var(--surface-2);
  ...
  .filter-chip{
    padding:9px 14px; border-radius:9px; background:var(--surface-2); border:1px solid var(--border);
    color:var(--text-dim); font-size:12.5px; cursor:pointer; font-weight:600;
  }
  .filter-chip.active{ background: rgba(129,140,248,0.16); border-color: rgba(129,140,248,0.5); color:var(--indigo); }

  table{ width:100%; border-collapse:collapse; }
  thead th{
    text-align:left; font-size:11px; text-transform:uppercase; letter-spacing:.6px;
    color:var(--text-faint); padding:12px 20px; border-bottom:1px solid var(--border); font-weight:600;
  }
  tbody td{ padding:14px 20px; border-bottom:1px solid var(--border); font-size:13px; vertical-align:middle;}
  tbody tr{ transition: background .15s ease; }
  tbody tr:hover{ background: rgba(255,255,255,0.02); }
  tbody tr:last-child td{ border-bottom:none; }
  tbody tr.key-enter{ animation: rowIn .4s ease; }
  @keyframes rowIn{ from{ opacity:0; transform:translateY(-6px);} to{ opacity:1; transform:translateY(0);} }

  .key-cell{ display:flex; align-items:center; gap:10px; }
  .key-text{ font-size:13px; letter-spacing:.5px; }
  .status-dot{ width:8px; height:8px; border-radius:50%; flex-shrink:0; }
  .status-dot.unused{ background:var(--amber); box-shadow:0 0 0 3px rgba(251,191,36,0.16);}
  .status-dot.active{ background:var(--emerald); box-shadow:0 0 0 3px rgba(52,211,153,0.16); animation: blink 1.8s infinite;}
  .status-dot.expired{ background:var(--rose); box-shadow:0 0 0 3px rgba(251,113,133,0.16);}
  .badge{ display:inline-flex; align-items:center; gap:6px; padding:5px 10px; border-radius:999px; font-size:11.5px; font-weight:700; }
  .badge.unused{ background:rgba(251,191,36,0.13); color:var(--amber); }
  .badge.active{ background:rgba(52,211,153,0.13); color:var(--emerald); }
  .badge.expired{ background:rgba(251,113,133,0.13); color:var(--rose); }
  .action-group{ display:flex; gap:6px; flex-wrap:wrap; }
  .ip-text{ color:var(--text-dim); font-size:12.5px; }
  .ip-text.none{ color:var(--text-faint); font-style:italic; }
  .lock-tag{ font-size:10.5px; color:var(--violet); background:rgba(167,139,250,0.12); padding:2px 7px; border-radius:6px; margin-left:6px;}
  .empty-state{ padding:60px 20px; text-align:center; color:var(--text-faint); }
  .empty-state .e-icon{ margin-bottom:14px; display:flex; justify-content:center; }
  .kv-row .v{ display:inline-flex; align-items:center; gap:6px; }
  .ip-registry-row{ display:flex; align-items:center; justify-content:space-between; padding:12px 0; border-bottom:1px dashed var(--border); font-size:12.5px; }
  .ip-registry-row:last-child{ border-bottom:none; }
  .ip-registry-row .rk{ display:flex; flex-direction:column; gap:2px; }
  .config-empty{ color:var(--text-faint); font-size:12.5px; text-align:center; padding:24px 0; }
  ...
  .modal{
    width:100%; max-width:460px; background: linear-gradient(165deg, var(--surface), var(--surface-2));
    border:1px solid var(--border); border-radius:18px; box-shadow: var(--shadow-black);
  ...
  .modal-head h3{ margin:0; font-size:15.5px; font-family:'Space Grotesk', sans-serif;}
  .modal-close{ background:none; border:none; color:var(--text-faint); font-size:20px; cursor:pointer; line-height:1;}
  .modal-body{ padding:20px 22px; }
  .kv-row{ display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px dashed var(--border); font-size:13px; }
  ...
  .config-box{
    ...
    font-size:12px; line-height:1.6; white-space:pre-wrap; word-break:break-all; color:var(--cyan);
    max-height:220px; overflow:auto;
  }
  .modal-foot{ padding:16px 22px; border-top:1px solid var(--border); display:flex; gap:10px; justify-content:flex-end;}

  #toastHost{ position:fixed; bottom:22px; right:22px; display:flex; flex-direction:column; gap:10px; z-index:300; }
  .toast{
    background: var(--surface); border:1px solid var(--border); border-left:3px solid var(--indigo);
    padding:12px 16px; border-radius:10px; box-shadow: var(--shadow-black); font-size:13px;
    display:flex; align-items:center; gap:10px; min-width:220px; animation: toastIn .25s ease;
  }
...
<div id="app">
  <header class="top">
    <div class="brand">
      <div>
        <h1 class="display">KeyForge <span style="color:var(--indigo)">Admin</span></h1>
        <span>Quản lý key · Bảo mật · Dữ liệu thật từ keys.json</span>
      </div>
    </div>
    <div class="top-right">
      <a class="btn btn-ghost btn-sm" href="/admin" style="text-decoration:none;">← Admin Panel</a>
      <div class="live-pill"><span class="live-dot"></span> Hôm nay: <span id="dailyCount">— / —</span></div>
  ...
  <section class="stats">
    <div class="stat-card total">
      <div class="stat-top"><div class="stat-icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2 2 7l10 5 10-5-10-5Z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg></div><div class="stat-trend">Tổng số</div></div>
      <div class="stat-value display" id="statTotal">0</div>
      <div class="stat-label">Tổng số Key</div>
    </div>
    <div class="stat-card unused">
      <div class="stat-top"><div class="stat-icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="2.5" fill="currentColor" stroke="none"/></svg></div><div class="stat-trend">Sẵn sàng</div></div>
      <div class="stat-value display" id="statUnused">0</div>
      <div class="stat-label">Key chưa dùng</div>
    </div>
    <div class="stat-card active">
      <div class="stat-top"><div class="stat-icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg></div><div class="stat-trend">Realtime</div></div>
      <div class="stat-value display" id="statActive">0</div>
      <div class="stat-label">Key đang hoạt động</div>
    </div>
    <div class="stat-card used">
      <div class="stat-top"><div class="stat-icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="m8.5 12.5 2.5 2.5 4.5-5"/></svg></div><div class="stat-trend">Đã kích hoạt</div></div>
      <div class="stat-value display" id="statUsed">0</div>
      <div class="stat-label">Key đã dùng</div>
    </div>
  </section>

  <div class="tabs">
    <button class="tab-btn active" data-tab="list" onclick="switchTab('list')">
      <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 10h18"/></svg>
      Danh sách Key
    </button>
    ...
    <button class="tab-btn" data-tab="ip" onclick="switchTab('ip')">
      <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M3 12h18"/><path d="M12 3c2.5 2.7 3.8 6 3.8 9s-1.3 6.3-3.8 9c-2.5-2.7-3.8-6-3.8-9s1.3-6.3 3.8-9Z"/></svg>
      Kiểm tra IP
    </button>
  </div>
  ...
  <div class="main-grid">
    <div class="panel">
      <div class="panel-head"><h2>Tạo Key mới</h2></div>
      <div class="panel-body">
        <div class="field">
          <label>Ghi chú / Khách hàng</label>
          <input type="text" id="inNote" placeholder="VD: Khách hàng A">
        </div>
        <div class="field">
        ...
        <div class="row-2">
          <div class="field">
            ...
            <input type="number" id="inMaxDevices" value="1" min="1" max="50">
          </div>
          <div class="field">
            <label>Số lượng tạo</label>
            <input type="number" id="inCount" value="1" min="1" max="20">
          </div>
        </div>
        <div class="field">
        ...
        <div class="switch-row">
          <div>
            <div class="label">Khoá theo IP</div>
            <div class="desc">Key chỉ dùng được từ 1 địa chỉ IP</div>
          </div>
          <label class="switch"><input type="checkbox" id="inLockIp"><span class="slider"></span></label>
        </div>
        <button class="btn btn-primary full-btn" onclick="createKeys()">＋ Tạo Key</button>
      </div>
    </div>
    ...
    <div class="panel">
      <div class="panel-head">
        <h2>Danh sách Key</h2>
        <span style="font-size:12px;color:var(--text-faint)" id="listCount">0 key</span>
      </div>
      <div class="table-toolbar">
        <div class="search-box">
          ...
          <input type="text" id="searchInput" placeholder="Tìm theo key hoặc ghi chú..." oninput="renderTable()">
        </div>
        <div class="filter-chip active" data-filter="all" onclick="setFilter('all')">Tất cả</div>
        <div class="filter-chip" data-filter="unused" onclick="setFilter('unused')">Chưa dùng</div>
        <div class="filter-chip" data-filter="active" onclick="setFilter('active')">Hoạt động</div>
        <div class="filter-chip" data-filter="expired" onclick="setFilter('expired')">Hết hạn</div>
      </div>
      ...
      <div class="empty-state" id="emptyState" style="display:none;">
        <div class="e-icon"><svg viewBox="0 0 24 24" width="34" height="34" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M22 12h-6l-2 3h-4l-2-3H2"/><path d="M5.5 5h13l3 7v7a2 2 0 0 1-2 2H4.5a2 2 0 0 1-2-2v-7l3-7Z"/></svg></div>
        Chưa có key nào — tạo key đầu tiên ở panel bên trái.
      </div>
    </div>
  </div>
  </div>
  ...
  <div class="tab-content" id="tab-config">
    <div class="panel narrow-panel">
      <div class="panel-head"><h2 style="display:flex;align-items:center;gap:9px;"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--indigo);"><line x1="21" y1="4" x2="14" y2="4"/><line x1="10" y1="4" x2="3" y2="4"/><line x1="21" y1="12" x2="12" y2="12"/><line x1="8" y1="12" x2="3" y2="12"/><line x1="21" y1="20" x2="16" y2="20"/><line x1="12" y1="20" x2="3" y2="20"/><line x1="14" y1="2" x2="14" y2="6"/><line x1="8" y1="10" x2="8" y2="14"/><line x1="16" y1="18" x2="16" y2="22"/></svg> Get Config theo Key</h2></div>
      <div class="panel-body">
        <div class="field">
          <label>Chọn Key</label>
          <select id="configKeySelect" onchange="renderConfigTab()"></select>
        </div>
        <div class="field">
          <label>Cấu hình (JSON)</label>
          <div class="config-box" id="configTabBox">
            <div class="config-empty">Chọn một key ở trên để lấy cấu hình.</div>
          </div>
        </div>
        <button class="btn btn-primary full-btn" onclick="copyConfigTab()">
  ...
  <div class="tab-content" id="tab-ip">
    <div class="main-grid">
      <div class="panel">
        <div class="panel-head"><h2>Kiểm tra / Gán IP</h2></div>
        <div class="panel-body">
          <div class="field">
            <label>Chọn Key</label>
            <select id="ipKeySelect" onchange="renderIpTabInfo()"></select>
          </div>
          <div class="kv-row"><span class="k">Khoá IP</span><span class="v" id="ipTabLock">—</span></div>
          <div class="kv-row"><span class="k">IP đã gán</span><span class="v" id="ipTabCurrent">—</span></div>
          <div class="field" style="margin-top:18px;">
            <label>Nhập IP cần kiểm tra / gán</label>
            <input type="text" id="ipTabInput" placeholder="VD: 118.70.22.14">
          </div>
          <div id="ipTabResult" style="min-height:20px;margin-bottom:6px;font-size:13px;"></div>
          <button class="btn btn-primary full-btn" onclick="runIpCheckTab()">Kiểm tra IP</button>
        </div>
      </div>
      <div class="panel">
        <div class="panel-head"><h2>Danh sách IP đã đăng ký</h2>
          <span style="font-size:12px;color:var(--text-faint)" id="ipRegistryCount">0 key</span>
        </div>
        <div class="panel-body" id="ipRegistryBody" style="padding:8px 20px;"></div>
      </div>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="checkModal">
  <div class="modal">
    <div class="modal-head"><h3>Kiểm tra Key</h3><button class="modal-close" onclick="closeModal('checkModal')">×</button></div>
    <div class="modal-body" id="checkModalBody"></div>
    <div class="modal-foot"><button class="btn btn-ghost btn-sm" onclick="closeModal('checkModal')">Đóng</button></div>
  </div>
</div>
...
function mapKey(item){
  return {
    ...
    expiresAt: (item.expires_at === undefined) ? null : item.expires_at,
    duration: item.duration,
    lockIp: !!item.lock_ip,
    ip: item.assigned_ip || null,
    used: !!item.used,
    keyType: item.key_type || 'premium',
    maxDevices: item.max_devices || 1
  };
}

function getStatus(k){
  if(k.expiresAt !== null && k.expiresAt !== undefined && k.expiresAt < Date.now()) return 'expired';
  if(!k.used) return 'unused';
  return 'active';
}
function timeRemaining(k){
  const status = getStatus(k);
  if(status === 'expired') return 'Đã hết hạn';
  if(k.expiresAt === null || k.expiresAt === undefined) return 'Vĩnh viễn';
  if(status === 'unused') return `Chưa dùng · còn ${formatDuration(k.expiresAt - Date.now())}`;
  return `Còn lại ${formatDuration(k.expiresAt - Date.now())}`;
}
...
async function loadStats(){
  try{
    const res = await fetch('/api/admin/today-count');
    const d = await res.json();
    document.getElementById('dailyCount').textContent = d.count + ' / ' + d.limit;
  }catch(e){}
}
      ...
      <td>
        <div class="key-cell">
          ...
          <div>
            ...
            <div style="font-size:11.5px;color:var(--text-faint);">${k.note || 'Không ghi chú'} · ${k.keyType==='vip'?'VIP':'Premium'}</div>
          </div>
        </div>
      </td>
...
async function deleteKey(id){
  ...
  try{
    ...
    if(res.ok){
      ...
      toast(data.error || 'Không thể xoá key', 'err');
    }
  }catch(e){ toast('Lỗi kết nối máy chủ', 'err'); }
}

async function createKeys(){
  const note = document.getElementById('inNote').value.trim();
  const duration = document.getElementById('inDuration').value;
  const maxDevices = document.getElementById('inMaxDevices').value;
  const quantity = document.getElementById('inCount').value;
  const keyType = document.getElementById('inKeyType').value;
  const lockIp = document.getElementById('inLockIp').checked;
  try{
    const res = await fetch('/api/admin/create', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({duration, max_devices: maxDevices, note, quantity, key_type: keyType, lock_ip: lockIp})
    });
    const data = await res.json();
    if(data.error && !data.keys){ toast(data.error, 'err'); return; }
    document.getElementById('inNote').value = '';
    await Promise.all([loadKeys(), loadStats()]);
    const n = (data.keys || []).length;
    toast(data.error ? `Đã tạo được ${n} key trước khi: ${data.error}` : `Đã tạo thành công ${n} key mới`, data.error ? 'err' : 'ok');
  }catch(e){ toast('Lỗi kết nối máy chủ', 'err'); }
}
...
function openCheck(id){
  ...
  document.getElementById('checkModalBody').innerHTML = `
    ...
    <div class="kv-row"><span class="k">Loại</span><span class="v">${k.keyType==='vip'?'VIP':'Premium'}</span></div>
    ...
    <div class="kv-row"><span class="k">Hết hạn lúc</span><span class="v">${k.expiresAt ? new Date(k.expiresAt).toLocaleString('vi-VN') : 'Vĩnh viễn'}</span></div>
    <div class="kv-row"><span class="k">Số thiết bị tối đa</span><span class="v">${k.maxDevices}</span></div>
...
async function runIpCheckTab(){
  ...
  try{
    const res = await fetch('/api/admin/verify-ip/'+k.id, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ip: ipVal})
    });
    const data = await res.json();
    if(!res.ok){ resultEl.innerHTML = `<span style="color:var(--rose)">${data.error||'Lỗi không xác định'}</span>`; return; }

    if(!data.locked){
      resultEl.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px;color:var(--amber)">${ICON.xCircle} Key này không khoá IP — mọi địa chỉ IP đều được chấp nhận.</span>`;
    } else if(data.just_assigned){
      resultEl.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px;color:var(--emerald)">${ICON.checkCircle} Đã gán IP ${ipVal} cho key này.</span>`;
    } else if(data.match){
      resultEl.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px;color:var(--emerald)">${ICON.checkCircle} IP khớp — key hợp lệ để sử dụng.</span>`;
    } else {
      resultEl.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px;color:var(--rose)">${ICON.xCircle} IP không khớp với IP đã đăng ký (${data.assigned_ip}).</span>`;
    }
    await loadKeys();
    const refreshed = findKey(k.id);
    document.getElementById('ipTabCurrent').textContent = refreshed ? (refreshed.ip || 'Chưa gán') : '—';
    renderIpRegistry();
  }catch(e){ resultEl.innerHTML = `<span style="color:var(--rose)">Lỗi kết nối máy chủ.</span>`; }
}
