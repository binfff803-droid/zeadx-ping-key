...
# Lưu trữ tạm link2 cho chế độ 24H
redirect_map = {}
...
def create_new_key(duration_type):
    ...
    if count_created_today(keys) >= DAILY_LIMIT:
        return None, "Đã đạt giới hạn 500 key/ngày"
    duration_map = {
        "12h": 12 * 60 * 60 * 1000,
        "24h": 24 * 60 * 60 * 1000,
        "1d": 24 * 60 * 60 * 1000,
...
def redirect_link(key):
    link2 = redirect_map.get(key)
    if link2:
        # Xóa sau khi dùng
        del redirect_map[key]
...
def index():
    ...
    <script>
      ...
      const opts=document.querySelectorAll('.duration-opt'); const genBtn=document.getElementById('genBtn'); const genSpinner=document.getElementById('genSpinner'); const genBtnLabel=document.getElementById('genBtnLabel');
...
def api_generate():
    ...
    if duration_type == "24h":
        ...
        # Lưu link2 vào map
        redirect_map[key] = link2
        # Tạo link1 (short) trỏ đến route /redirect/<key>
        redirect_route = f"{host}/redirect/{key}"
