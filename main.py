...
# QUAN TRỌNG: không còn giá trị mặc định hard-code ở đây nữa.
# Bạn PHẢI set ADMIN_USERNAME và ADMIN_PASSWORD trong Environment Variables
# trên Render bằng giá trị MỚI (khác với "Zeadxvnstore" / "zeadxvn10101" cũ,
# vì 2 giá trị cũ đã từng nằm trong repo Public nên coi như đã bị lộ).
...
# ---- Admin Key Panel: xác thực bằng X-Admin-Key (dùng cho panel deploy domain riêng) ----
import hmac

ADMIN_API_KEY = os.environ.get("ADMIN_API_KEY", "")  # BẮT BUỘC set trên Render, để trống = tắt tính năng


def _valid_api_key(req):
    if not ADMIN_API_KEY:
        return False
    sent = req.headers.get("X-Admin-Key", "")
    return hmac.compare_digest(sent, ADMIN_API_KEY)


def admin_auth_required(f):
    """Chấp nhận admin đã login bằng session (web cũ) HOẶC bằng X-Admin-Key (panel riêng)."""
    from functools import wraps
    @wraps(f)
    def wrapper(*args, **kwargs):
        if session.get("is_admin") or _valid_api_key(request):
            return f(*args, **kwargs)
...
# ============ Admin Key Panel API (bổ sung) ============
# Bật CORS có kiểm soát cho các route admin API (đã có CORS(app) toàn cục rồi,
# nhưng để rõ ràng và an toàn hơn nếu sau này bạn siết ALLOWED_ORIGINS lại).
...
@app.route("/api/admin/keys/<int:key_id>", methods=["PATCH"])
@admin_auth_required
def admin_update_key(key_id):
    """Sửa note / bật-tắt khoá IP cho 1 key."""
    keys = load_keys()
    k = _find_key(keys, key_id)
    if not k:
        return jsonify({"error": "Không tìm thấy key"}), 404
    data = request.get_json(force=True, silent=True) or {}
    if "note" in data:
        k["note"] = str(data["note"])[:200]
    if "lock_ip" in data:
        k["lock_ip"] = bool(data["lock_ip"])
        if not k["lock_ip"]:
            k["ip"] = None
    save_keys(keys)
    return jsonify(k)
...
def admin_activate_key(key_id):
    """Kích hoạt key lần đầu (set activated_at + expires_at theo duration)."""
    keys = load_keys()
    k = _find_key(keys, key_id)
    if not k:
        return jsonify({"error": "Không tìm thấy key"}), 404
    if not k.get("activated_at"):
        duration_map = {
            "12h": 12 * 60 * 60 * 1000, "24h": 24 * 60 * 60 * 1000, "1d": 24 * 60 * 60 * 1000,
            "3d": 3 * 24 * 60 * 60 * 1000, "7d": 7 * 24 * 60 * 60 * 1000, "forever": None
        }
        ms = duration_map.get(k.get("duration"))
        now = time.time() * 1000
        k["activated_at"] = now
        k["expires_at"] = (now + ms) if ms is not None else None
        save_keys(keys)
    return jsonify(k)


@app.route("/api/admin/keys/<int:key_id>/check-ip", methods=["POST"])
@admin_auth_required
def admin_check_ip(key_id):
    """
    Kiểm tra / gán IP cho key:
    - Nếu key không khoá IP -> luôn hợp lệ, ghi nhận IP gửi lên.
    - Nếu khoá IP mà chưa có IP nào gán -> gán IP này.
    - Nếu khoá IP và đã có IP -> so khớp.
    """
    keys = load_keys()
    k = _find_key(keys, key_id)
    if not k:
    ...
    if not k.get("lock_ip"):
        k["ip"] = ip
        save_keys(keys)
        return jsonify({"result": "no_lock", "message": "Key không khoá IP — mọi IP đều hợp lệ.", "ip": ip})

    if not k.get("ip"):
        k["ip"] = ip
        save_keys(keys)
        return jsonify({"result": "assigned", "message": f"Đã gán IP {ip} cho key này.", "ip": ip})

    if k["ip"] == ip:
        return jsonify({"result": "match", "message": "IP khớp — key hợp lệ.", "ip": ip})

    return jsonify({"result": "mismatch", "message": f"IP không khớp với IP đã đăng ký ({k['ip']}).", "ip": k["ip"]})


@app.route("/api/admin/keys/<int:key_id>/config", methods=["GET"])
@admin_auth_required
def admin_key_config(key_id):
    """Trả về config JSON của 1 key (dùng cho tab Config trên panel)."""
    keys = load_keys()
    k = _find_key(keys, key_id)
    if not k:
        return jsonify({"error": "Không tìm thấy key"}), 404
    now = time.time() * 1000
    if k.get("activated_at") and k.get("expires_at"):
        status = "expired" if k["expires_at"] < now else "active"
    elif k.get("activated_at") and k.get("expires_at") is None:
